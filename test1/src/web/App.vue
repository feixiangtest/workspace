<template>
  <ionside>
      ...
      <!--左侧导航栏 -->
      <drawer :open="openLeft" :docked="dockedLeft" @close="toggle()">
        <lotteryMenu></lotteryMenu>
      </drawer>

      <!--右侧信息栏 -->
      <drawer right :open="openR" :docked="dockedRight" @close="toggle(true)">
...
import lotteryMenu from '@/web/components/leftSideMenu/leftSideMenu'
import rightSideMenu from '@/web/components/rightSideMenu/rightSideMenu'
...
export default {
  ...
  mounted () {
    this.$nextTick(() => { // 移动导航到对应的位置
      window.$('#preApp').stop().animate({opacity: 0}, 10)
      setTimeout(() => {
