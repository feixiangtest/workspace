...
import * as klsfService from 'service/betService/klsfService'
export default {
  data () {
    return {
      logins: true, // 是否登录
  ...
  methods: {
    ...
    openLuzi () { // 显示路子长龙
      this.$store.state.betZone.klsfLuziShow = true
    },
    reset () { // 重置筹码
      this.money = ''
      this.hides = true
    ...
    orderMethod () { // 校验玩法
      ...
      if (this.isLogin) {
        ...
        // 判断连码
        if (this.nowModel === 2591000) {
          let isTrueByLianMa = klsfService.isTrueByLianMa(this.$store.state.betZone.selectedRule, this.$store.state.betZone.selectedList)
          if (!isTrueByLianMa) return false
        }

        // 拼接数据
        if (this.nowModel === 2591000) { // 连码
          this.$store.state.betZone.doBetList = klsfService.newDobetList(this.$store.state.betZone.selectedRule, this.$store.state.betZone.selectedList)
        } else { // 其它玩法，比如“双面盘”、“单球1~8”
          this.$store.state.betZone.doBetList = this.$store.state.betZone.selectedList
        }

        for (let i = 0; i < this.$store.state.betZone.doBetList.length; i++) { // 将金额放入到对象中
