          ...
          <td style="padding-left: 0px;padding-right: 0px;">
              <p>
                ...
                <span v-if="!isNaN(item.project)">
                    <span v-if="item.typeName === '特码'">{{item.typeName}}
                      <i class="ball-c ball-c-xs" 
                        :class="{
                                'ball-c2':item.project%3==1 && item.project!=14 && item.project!=0 && item.project!=13 && item.project!=27,
                                'ball-c3':item.project%3==2 && item.project!=14 && item.project!=0 && item.project!=13 && item.project!=27,
                                'ball-c4':item.project%3==0 && item.project!=14 && item.project!=0 && item.project!=13 && item.project!=27,
                                'ball-c1':item.project==14 || item.project==0 || item.project==13 || item.project==27}">{{item.project}}
                      </i>
                    </span>
                </span>
              </p>
...
export default {
  ...
  methods: {
    ...
    deleteBet (index) {
      ...
      // 判断是否是特码，pcddSelect置空， tema_array 设置回默认[0,1,2]
      if (this.$store.state.betZone.doBetList[index].typeName === '特码包三') {
        this.$store.state.betZone.pcddSelect = {}
        console.info('pcddSelect:', this.$store.state.betZone.pcddSelect)
        ...
        this.$store.state.betZone.tema_array.push(2)
      }
