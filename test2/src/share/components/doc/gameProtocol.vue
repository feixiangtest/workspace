<template>
    <div v-if='showLoginNotice' class="zhezhao"  tabindex = "0" @keyup.enter="agreeMethod" id='loginNotice'>
        <div class="newLoginBox agreementBox">
            <div class="newLoginTitle">
                <span>游戏协议</span>
            </div>
            <div class="newAgreementCent">
                <div class="agreementMenu">
                    <ul class="clearfix">
                        <li v-for='(item,index) in protocolContentList' :key='index' @click="curTabIndex = index"
                            :id="index" :class="index === curTabIndex?'cur':''">
                            {{item.title}}
                        </li>
                    </ul>
                </div>
                <div class="agreementConten">
                    <div v-for="(item,index) in protocolContentList" :key="index" v-html="item.content"
                        :id="index" :class="index === curTabIndex?'cur':''" class="xy-c-main"
                        :style="index === curTabIndex?'display:block':'display:none'">
                    </div>
                </div>
            </div>
            <div class="newAgreementBot">
                <span @click="disagree">{{disagreeTitle}}</span>
                <span class="xy-yes" @click="agreeMethod">{{agreeTitle}}</span>
            </div>
        </div>
    </div>
</template>
<script>
    export default {
      props: ['showLoginNotice', 'agree', 'disagree'],
      data () {
        return {
          curTabIndex: window.common.const.curTabIndex,
          lastLoginTime: 0,
      ...
      updated () {
        if (document.getElementById('loginNotice')) {
          document.getElementById('loginNotice').focus()
        }
      },
      computed: {
        gameTitle () {
          return window.common.const.gameTitle
        },
        disagreeTitle () {
          return window.common.const.disagreeTitle
        },
        agreeTitle () {
          return window.common.const.agreeTitle
        }
      },
      methods: {
        agreeMethod () {
